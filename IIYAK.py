...
def get_availability_filename(event_id):
    """Generates a clean, consistent availability file name for each event ID."""
...
def get_entertainment_type():
    """Replaced recursive loop with an interactive loop to prevent crash loop."""
    while True:
...
def get_booking():
    ...
    while True:
        ...
        # Extract ID in case the user types 'E-001 Name Of Event'
        event_match = re.match(r'^(E-\d{3})', event_input, re.IGNORECASE)
        if event_match:
            event_id = event_match.group(1).upper()
        else:
            event_id = event_input.upper()

        availability = load_availability(event_id)
        if availability is not None:
            break
            
        print(f"No dynamic availability data found for Event ID: {event_id}. (Has the manager configured it?)")
        if os.path.exists(EVENT_LIST):
    ...
    if slots <= available_slots:
        # Pricing strategy: Section Selection * Slots * Base Ticket Price
