...
def get_availability_filename(event_id):
    """Generates a unique availability file for each event."""
    return f"availability_{event_id.upper().strip()}.txt"
...
def delete_event():
    ...
    # Clean up associated event availability file if it exists
...
def manager_set_availability():
    ...
    # Saved to event-specific tracking file
    avail_file = get_availability_filename(event_id)
...
def get_booking():
    ...
    # 1. Ask for the Event Code to look up custom definitions
    while True:
        event_id = input("Enter Event ID (e.g. E-001): ").upper().strip()
        availability = load_availability(event_id)
        if availability is not None:
            break
        print("No dynamic availability data found for this Event ID. (Has the manager configured it?)")
        if os.path.exists(EVENT_LIST):
    ...
    if slots <= available_slots:
        # 2. Total calculation formula: Section Number * Slots * Base Price
        ...
        # Update the event's unique availability file record
        avail_file = get_availability_filename(event_id)
        with open(avail_file, "w") as file:
        ...
        print(f"Total payment: ₱{total_payment:.2f} (Formula: Section {section_number} × {slots} Slots × ₱{availability['base_price']:.2f} Base)")
