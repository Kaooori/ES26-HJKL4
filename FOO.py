...
def manager_set_availability():
    ...
    # FIX: Dynamically scan layout file lines to secure Base Ticket Price accurately
    base_price = 0.0
    for line in event_lines:
        if "base ticket price" in line.lower():
            try:
                base_price = float(line.split(":")[-1].replace("₱", "").strip())
                break
            except ValueError:
                pass
...
def load_availability(event_id):
    """
    Called only AFTER the event is validated. Generates default availability
    by safely scanning lines for Base Price if tracking metadata is missing.
    """
    avail_file = get_availability_filename(event_id)
    ...
    if not os.path.exists(avail_file):
        ...
        # FIX: Dynamically scan master file lines to secure Base Ticket Price accurately
        base_price = 0.0
        for line in lines:
            if "base ticket price" in line.lower():
                try:
