...
def load_availability(event_id):
    ...
    if the manager hasn't linked a venue yet.
    """
    avail_file = get_availability_filename(event_id)
    master_event_file = f"{event_id}.txt"
    
    # If there is no specific availability file, construct defaults using the event master file.
        ...
        if not event_valid:
            ...
            if os.path.exists(EVENT_LIST):
                ...
                with open(EVENT_LIST, "r") as file:
                    print(file.read().strip())
                print("-------------------------")
            continue  # Re-prompts the user to enter Event ID

        # 2. Read the corresponding E-XXX.txt master file for Base Price/Name details
        master_event_file = f"{event_id}.txt"
        if not os.path.exists(master_event_file):
            print(f"Error: The event master file ({master_event_file}) is missing.")
            continue

        # Validated successfully. Load or generate availability mapping.
