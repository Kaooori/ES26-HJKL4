...
def get_base_price(event_id):
    """Safely extracts the base price from the master event file regardless of line position."""
    master_file = f"{event_id}.txt"
    base_price = 0.0
    if os.path.exists(master_file):
        with open(master_file, "r", encoding="utf-8") as f:
            for line in f:
                if "base ticket price" in line.lower():
                    # Regex to grab only the numbers, ignoring spaces or peso signs
...
def add_customer():
    ...
    with open(CUSTOMER_LIST, "a", encoding="utf-8") as file:
...
def show_all_customers():
    ...
    if not os.path.exists(CUSTOMER_LIST):
        ...
        return
    with open(CUSTOMER_LIST, "r", encoding="utf-8") as file:
        print(file.read())
...
def delete_customer():
    ...
    if os.path.exists(CUSTOMER_LIST):
        ...
        with open(CUSTOMER_LIST, "w", encoding="utf-8") as file:
...
def add_venue():
    ...
    with open(VENUE_LIST, "a", encoding="utf-8") as file:
...
def show_all_venues():
    ...
    if not os.path.exists(VENUE_LIST):
        ...
        return
    with open(VENUE_LIST, "r", encoding="utf-8") as file:
        print(file.read())
...
def delete_venue():
    ...
    if os.path.exists(VENUE_LIST):
        ...
        with open(VENUE_LIST, "w", encoding="utf-8") as file:
            file.writelines(updated_lines)
...
def add_event():
    ...
    with open(EVENT_LIST, "a", encoding="utf-8") as file:
...
def show_all_events():
    ...
    if not os.path.exists(EVENT_LIST):
        ...
        return
    with open(EVENT_LIST, "r", encoding="utf-8") as file:
        print(file.read())
...
def delete_event():
    ...
    if os.path.exists(EVENT_LIST):
        ...
        with open(EVENT_LIST, "w", encoding="utf-8") as file:
            file.writelines(updated_lines)
...
def manager_set_availability():
    ...
    with open(f"{event_id}.txt", "r", encoding="utf-8") as f:
        event_name = f.readline().strip()

    base_price = get_base_price(event_id)

    with open(f"{venue_code}.txt", "r", encoding="utf-8") as f:
...
def load_availability(event_id):
    """
    Called after event validation. Will auto-generate default mapping or self-heal
    base prices if it reads an old 0.0 file.
    """
    avail_file = get_availability_filename(event_id)
    master_event_file = f"{event_id}.txt"
    
    if not os.path.exists(avail_file):
        with open(master_event_file, "r", encoding="utf-8") as f:
            lines = f.readlines()
    ...
    try:
        b_price = float(data[4].strip())
    except (ValueError, IndexError):
        b_price = 0.0

    # SELF-HEAL FEATURE: If the old file had a 0.0 error, dynamically pull it from the real file
    if b_price == 0.0:
        b_price = get_base_price(event_id)
...
def get_booking():
    ...
    # 1. Ask for the Event Code and strictly validate it against Event List.txt
    while True:
        ...
        if os.path.exists(EVENT_LIST):
            with open(EVENT_LIST, "r", encoding="utf-8") as f:
                for line in f:
                    if line.upper().startswith(event_id) and "[DELETED EVENT]" not in line.upper():
                        event_valid = True
                        break
        
        if not event_valid:
            print("\nInvalid Event ID. This event is not in the events list.")
            if os.path.exists(EVENT_LIST):
        ...
        if not os.path.exists(master_event_file):
            print(f"Error: The event master file ({master_event_file}) is missing.")
            continue
    ...
    with open(f"{customer_id}.txt", "r", encoding="utf-8") as file:
    ...
    if slots <= available_slots:
        ...
        with open(f"{booking_id}.txt", "w", encoding="utf-8") as file:
            file.write(f"Customer ID: {customer_id}\n")
        ...
        with open(BOOKING_LIST, "a", encoding="utf-8") as file:
...
def display_all_bookings():
    ...
    if not os.path.exists(BOOKING_LIST):
        ...
        return
    with open(BOOKING_LIST, "r", encoding="utf-8") as file:
        print(file.read())
...
def delete_booking():
    ...
    if os.path.exists(BOOKING_LIST):
        ...
        with open(BOOKING_LIST, "w", encoding="utf-8") as file:
            file.writelines(updated_lines)
