...
def delete_venue():
    ...
    if os.path.exists(VENUE_LIST):
        with open(VENUE_LIST, "r") as file:
            lines = file.readlines()
        updated_lines = [line for line in lines if not line.startswith(venue_code)]
...
def type():
    ...
    try:
        choice3 = int(input("Entertainment Type: "))
    except ValueError:
        print("\n>> Invalid input.")
        return type()
...
def get_booking():
    ...
    if availability is None:
        print("No availability data configured by the manager.")
        return
    ...
    if slots <= available_slots:
        # FIXED: Multiplies base_price * slots * section_number as requested
        total_payment = availability["base_price"] * slots * section_number
...
def delete_booking():
    ...
    if os.path.exists(BOOKING_LIST):
        with open(BOOKING_LIST, "r") as file:
            lines = file.readlines()
        updated_lines = [line for line in lines if not line.startswith(booking_id)]
