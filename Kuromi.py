...
def get_next_id(prefix, list_file):
    prefix = prefix.upper()  # Ensure prefix is always uppercase
...
def get_entertainment_type():
    """Replaced recursive loop with a safe while-loop to prevent RecursionError"""
    while True:
...
def get_booking():
    ...
    if slots <= available_slots:
        total_payment = availability["base_price"] * slots
